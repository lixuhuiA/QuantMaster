...
import { QuantApi } from './services/api'; // 引入刚才写的 API 服务

// Helper icons (保持 UI 不变)
const Icons = {
...
const App: React.FC = () => {
  ...
  const loadAllData = async () => {
    setIsLoading(true);
    try {
      const [fetchedRecords, fetchedLoans, fetchedSettings] = await Promise.all([
        QuantApi.fetchRecords(),
        QuantApi.fetchLoans(),
        QuantApi.fetchSettings()
      ]);
  ...
  const sendNotification = async (record: MonthlyRecord) => {
    ...
    try {
      ...
      console.log(`Notification Mock: ${settings.wechatWebhookUrl} - Total: ${totalAssets}`);
    } catch (e) {
  ...
  const handleSaveRecord = async (record: MonthlyRecord) => {
    try {
        await QuantApi.saveRecord(record); // 调用 API
        await loadAllData(); // 重新加载以确保数据同步（包括 id 和关联表）
  ...
  const handleDeleteRecord = async (id: string) => {
    if (window.confirm("确定要删除这条记录吗？此操作不可恢复。")) {
      try {
          await QuantApi.deleteRecord(id); // 调用 API
  ...
  const handleSaveSettings = async (newSettings: AppSettings) => {
    try {
        await QuantApi.saveSettings(newSettings); // 调用 API
  ...
  const handleUpdateLoans = async (newLoans: LoanRecord[]) => {
      try {
          await QuantApi.saveLoans(newLoans); // 调用 API
          setLoans(newLoans);
      } catch (e) {
          alert("借款记录保存失败");
      }
      ...
      <main className="flex-1 overflow-y-auto relative custom-scrollbar p-4 md:p-6 bg-[#f1f5f9] mt-16 md:mt-0">
        <div className="max-w-7xl mx-auto relative z-10">
          {isLoading ? (
            ...
            <>
              {view === 'DASHBOARD' && <Dashboard data={records} settings={settings} />}
              {view === 'LOANS' && <LoanManager loans={loans} onUpdateLoans={handleUpdateLoans} onClose={() => setView('DASHBOARD')} />}
              {view === 'ENTRY' && (
              ...
              {view === 'SETTINGS' && <Settings settings={settings} onSave={handleSaveSettings} onCancel={() => setView('DASHBOARD')} />}
                   ...
                   <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 bg-white p-6 rounded-3xl shadow-soft border border-slate-200/60 gap-4">
                      ...
                      <div className="flex gap-2">
                         {settings.autoFillMissingMonths && (
                            <span className="text-xs font-mono bg-quant-brand/10 text-quant-brand px-3 py-1.5 rounded-lg font-bold border border-quant-brand/20">⚡ 自动补全</span>
                         )}
                        ...
                        <tbody className="divide-y divide-slate-50">
                          {[...records].reverse().map((record, index, arr) => {
                            ...
                            if (prevRecord) {
                               ...
                               theoreticalAssets = actualTotalAssets;
                            }
                              ...
                              <React.Fragment key={record.id}>
                                ...
                                >
                                  ...
                                  <td className="px-6 py-4 text-center">
                                    {isAllMet ? (
                                      <span className="inline-flex items-center gap-1 bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full text-[10px] font-bold border border-emerald-200">✅ 达标</span>
                                    ) : (
                                  ...
                                  <td className="px-6 py-4 text-right font-mono text-slate-500 font-medium">
                                    {prevRecord ? (
                                        <div className="flex flex-col items-end">
                                          ...
                                          {missingMonthsInfo && (
                                            <span className="text-[9px] text-quant-brand bg-blue-50 px-1 rounded mt-0.5">含{missingMonthsInfo}个月自动补全</span>
                                          )}
                                        </div>
                                    ) : '-'}
                                  </td>
                                  <td className="px-6 py-4 text-right font-mono text-blue-700 font-bold bg-blue-50/30">¥{actualTotalAssets.toLocaleString()}</td>
                                      ...
                                      <div className="flex flex-col md:flex-row gap-4 mb-6 text-xs border-b border-slate-200 pb-4">
                                         <div className="bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm flex items-center gap-3">
                                            ...
                                            <div className="flex gap-3 font-mono">
                                               <span className={isProvidentMet ? 'text-emerald-600' : 'text-rose-500'}>公: ¥{targetProvident} {isProvidentMet ? '✓' : '✗'}</span>
                                               <span className="text-slate-300">|</span>
                                               <span className={isCashMet ? 'text-blue-600' : 'text-rose-500'}>现: ¥{targetCash} {isCashMet ? '✓' : '✗'}</span>
                                            </div>
                                         </div>
                                         <div className="bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm flex items-center gap-3">
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-5 gap-8 text-xs">
                                        <div className="col-span-1 md:col-span-2 bg-white p-4 rounded-xl border border-blue-100">
                                          ...
                                          <div className="space-y-3 font-mono">
                                            <div className="flex justify-between items-center"><span className="text-slate-500">流动资金 (现金/卡/投资)</span><span className="font-bold text-slate-800">¥{(record.balanceLiquid + record.balanceInvestments).toLocaleString()}</span></div>
                                            ...
                                            <div className="flex justify-between items-center pt-2 border-t border-slate-100"><span className="text-emerald-600 font-bold">公积金 (Provident)</span><span className="font-bold text-emerald-600">¥{record.balanceProvident.toLocaleString()}</span></div>
                                            <div className="flex justify-between items-center"><span className="text-orange-500 font-bold">借出/应收 (Lent)</span><span className="font-bold text-orange-500">¥{(record.balanceLent || 0).toLocaleString()}</span></div>
                                          </div>
                                        </div>
